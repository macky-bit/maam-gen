...
export function CatalogView({ data, onWatch, onInfo }: CatalogViewProps) {
  ...
  const genres = useMemo(() => {
    const values = data.rows.flatMap((row) => row.shows.flatMap((show) => show.genres));
    ...
    <main>
      {data.loading && (
        <div className="flex min-h-[78vh] items-center justify-center">
          <p className={`text-sm animate-pulse ${styles.loadingText}`}>Loading TMDB titles…</p>
        </div>
      )}

      {data.error && !data.loading && (
        <div className="flex min-h-[78vh] flex-col items-center justify-center gap-3 px-6 text-center">
          <p className={`text-sm font-semibold ${styles.errorText}`}>{data.error}</p>
          <p className={`text-xs ${styles.errorHint}`}>Add your TMDB key to VITE_TMDB_API_KEY and try again.</p>
        </div>
      )}
