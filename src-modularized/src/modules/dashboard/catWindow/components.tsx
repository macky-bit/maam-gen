...
import styles from './catWindow.module.css'
...
function Navbar({
  ...
  return (
    <nav
      ...
      style={{ backgroundColor: 'var(--color-ink)', borderBottom: '1px solid var(--color-stone)' }}
    >
      {/* Logo */}
      <span
      ...
      >
        STREAM<span style={{ color: 'var(--color-wine)' }}>FLIX</span>
      </span>
      ...
      <div className="ml-auto flex items-center gap-3">
        ...
        {searchOpen ? (
          <div
            ...
            style={{ border: '1px solid var(--color-wine)', backgroundColor: 'var(--color-ink)' }}
          >
            <svg className="w-4 h-4 flex-shrink-0" style={{ color: 'var(--color-taupe)' }} fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
            ...
            <input
              ...
              style={{ color: 'var(--color-cream)', caretColor: 'var(--color-cream)' }}
        ...
        <div className="relative">
          ...
          {notifOpen && (
            <div
              ...
              style={{ backgroundColor: 'var(--color-ink-soft)', border: '1px solid var(--color-stone)' }}
            >
              <div className="px-4 py-3" style={{ borderBottom: '1px solid var(--color-stone)' }}>
                <p className="font-semibold text-sm" style={{ color: 'var(--color-cream)', fontFamily: "'Barlow Condensed', sans-serif", letterSpacing: '0.05em' }}>
                  NOTIFICATIONS
                </p>
              </div>
              {[
                ...
                <div
                  ...
                  style={{ borderBottom: i < 2 ? '1px solid var(--color-stone)' : 'none' }}
                  onMouseEnter={e => (e.currentTarget.style.backgroundColor = 'var(--color-cream)')}
                  onMouseLeave={e => (e.currentTarget.style.backgroundColor = 'transparent')}
                >
                  <div
                    className="w-2 h-2 mt-1.5 rounded-full flex-shrink-0"
                    style={{ backgroundColor: n.unread ? 'var(--color-wine)' : 'var(--color-stone)' }}
                  />
                  <div>
                    <p className="text-xs font-semibold" style={{ color: 'var(--color-cream)' }}>{n.title}</p>
                    <p className="text-xs" style={{ color: 'var(--color-taupe)' }}>{n.msg}</p>
                    <p className="text-[10px] mt-0.5" style={{ color: 'var(--color-stone)' }}>{n.time}</p>
                  </div>
                </div>
              ))}
            </div>
              ...
              {['Manage Profiles', 'Transfer Profile', 'Account', 'Help Center', 'Sign out of StreamFlix'].map((item, i, arr) => (
                <a
                  ...
                  style={{
                    ...
                    borderBottom: i < arr.length - 1 ? '1px solid var(--color-stone)' : 'none',
                  }}
                  onMouseEnter={e => (e.currentTarget.style.color = 'var(--color-cream)')}
                  onMouseLeave={e => (e.currentTarget.style.color = 'var(--color-taupe)')}
                >
    ...
    >
      <div
        ...
        style={{
          ...
          zIndex: hovered ? 10 : 1,
          boxShadow: hovered ? '0 8px 32px var(--color-ink)' : 'none',
        }}
      >
        ...
        {hovered && (
          <div
            ...
            style={{ background: 'linear-gradient(to top, var(--color-ink) 0%, var(--color-ink) 50%, transparent 100%)' }}
          >
            <p
            ...
            <div className="flex items-center gap-1.5 mb-2">
              <span className="text-[11px] font-semibold" style={{ color: 'var(--color-ink-soft)' }}>{item.match}%</span>
              <span className="text-[10px]" style={{ color: 'var(--color-taupe)' }}>{item.year}</span>
              <span
                className="text-[9px] px-1 border"
                style={{ color: 'var(--color-taupe)', borderColor: 'var(--color-stone)' }}
              >
                {item.rating}
              </span>
            </div>
            <div className="flex items-center gap-1.5">
              ...
              <button
                ...
                style={{
                  border: `1.5px solid ${inMyList ? 'var(--color-cream)' : 'var(--color-stone)'}`,
                  color: inMyList ? 'var(--color-ink-soft)' : 'var(--color-cream)',
                }}
              ...
              <button
                className="w-7 h-7 rounded-full flex items-center justify-center flex-shrink-0 transition-all hover:border-[var(--color-cream)]"
                style={{ border: '1.5px solid var(--color-stone)', color: 'var(--color-cream)' }}
                title="Like"
              >
                <ThumbUpIcon size={11} />
              </button>
              <button
                ...
                className="w-7 h-7 rounded-full flex items-center justify-center flex-shrink-0 transition-all hover:border-[var(--color-cream)] ml-auto"
                style={{ border: '1.5px solid var(--color-stone)', color: 'var(--color-cream)' }}
                title="More Info"
              >
    ...
    >
      ...
      <span
        ...
        style={{
          ...
          WebkitTextStroke: '2.5px var(--color-stone)',
          color: 'transparent',
          lineHeight: 0.82,
          zIndex: 1,
      ...
      <div
        ...
        style={{
          ...
          boxShadow: hovered ? '0 6px 24px var(--color-ink)' : '2px 2px 12px var(--color-ink)',
        }}
      >
        ...
        {/* Badge */}
        {item.badge && !hovered && (
          <div className="absolute bottom-0 left-0 right-0 px-1.5 py-1" style={{ backgroundColor: 'var(--color-ink)' }}>
            <Badge label={item.badge} />
          </div>
        )}
      ...
      {Array.from({ length: total }).map((_, i) => (
        <span
          ...
          style={{
            ...
            height: '2px',
            backgroundColor: i === active ? 'var(--color-cream)' : 'var(--color-stone)',
          }}
        />
      ))}
    </div>
  ...
  return (
    <section className="mb-8" onMouseEnter={() => setRowHovered(true)} onMouseLeave={() => setRowHovered(false)}>
      ...
      <div className="relative">
        ...
        <div
          ref={scrollRef}
          className={`flex gap-2 overflow-x-auto px-6 md:px-12 pb-4 ${styles.noScrollbar}`}
  ...
  return (
    <section className="mb-10" onMouseEnter={() => setRowHovered(true)} onMouseLeave={() => setRowHovered(false)}>
      ...
      <div className="relative">
        ...
        <div
          ref={scrollRef}
          className={`flex gap-1 overflow-x-auto px-6 md:px-12 pb-4 ${styles.noScrollbar}`}
          onScroll={updateState}
          style={{ scrollSnapType: 'x proximity' }}
        >
          {items.map((item, i) => (
      ...
      >
        ...
        <div className="relative h-64 md:h-72">
          ...
          <div
            ...
            style={{ background: 'linear-gradient(to top, var(--color-ink-soft) 0%, var(--color-ink-soft) 60%, transparent 100%)' }}
          />
          {/* Close button */}
          <button
            onClick={onClose}
            ...
            onMouseEnter={e => (e.currentTarget.style.backgroundColor = 'var(--color-ink)')}
        ...
        <div className="px-6 pt-4 pb-2">
          <div className="flex items-center gap-3 mb-4">
            ...
            <button
              ...
              style={{
                border: `2px solid ${inMyList ? 'var(--color-ink-soft)' : 'var(--color-stone)'}`,
                color: inMyList ? 'var(--color-ink-soft)' : 'var(--color-cream)',
              }}
              title={inMyList ? 'Remove from My List' : 'Add to My List'}
            >
            ...
            <button
              className="w-9 h-9 rounded-full flex items-center justify-center transition-all"
              style={{ border: '2px solid var(--color-stone)', color: 'var(--color-cream)' }}
              onMouseEnter={e => (e.currentTarget.style.borderColor = 'var(--color-cream)')}
          ...
          <div className="flex items-center gap-3 mb-4 flex-wrap">
            <span className="text-sm font-semibold" style={{ color: 'var(--color-ink-soft)' }}>{item.match}% Match</span>
            <span className="text-sm" style={{ color: 'var(--color-cream)' }}>{item.year}</span>
            <span
            ...
            <span className="text-sm" style={{ color: 'var(--color-taupe)' }}>{item.duration}</span>
          </div>

          <p className="text-sm leading-relaxed mb-3" style={{ color: 'var(--color-cream)' }}>
            {item.description}
          </p>

          <p className="text-xs mb-1">
            <span style={{ color: 'var(--color-stone)' }}>Genres: </span>
            <span style={{ color: 'var(--color-taupe)' }}>{item.genres.join(', ')}</span>
          </p>
          <p className="text-xs">
            <span style={{ color: 'var(--color-stone)' }}>Type: </span>
            <span style={{ color: 'var(--color-taupe)' }}>{item.type === 'movie' ? 'Movie' : 'TV Show'}</span>
          </p>
        </div>

        {/* Similar titles */}
        {similar.length > 0 && (
          <div className="px-6 py-5" style={{ borderTop: '1px solid var(--color-stone)' }}>
            <h3
            ...
            <div className="grid grid-cols-3 gap-2">
              {similar.map(s => (
                ...
                >
                  ...
                  <div className="p-1.5">
                    <p className="text-xs font-semibold truncate" style={{ color: 'var(--color-cream)' }}>{s.title}</p>
                    <p className="text-[10px]" style={{ color: 'var(--color-taupe)' }}>{s.year} · {s.genres[0]}</p>
                  </div>
                </div>
              ))}
            </div>
...
export function CatWindowView() {
  ...
  return (
    <div
      className={`min-h-screen ${styles.page}`}
      onClick={e => {
