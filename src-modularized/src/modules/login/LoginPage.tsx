          ...
          <form onSubmit={handleSubmit} noValidate className="flex flex-col gap-5">
            ...
            <div className="flex items-center justify-between">
              <label className="flex items-center gap-2 cursor-pointer select-none">
                <input type="checkbox" checked={remember} onChange={(e) => setRemember(e.target.checked)} className="w-4 h-4 accent-[var(--color-wine)] rounded" />
                <span className={`text-xs ${styles.rememberText}`}>Remember me</span>
              </label>
              <button type="button" className={`text-xs underline underline-offset-2 transition-colors hover:text-[var(--color-cream)] ${styles.forgotBtn}`}>
                Forgot Password?
              </button>
            </div>
