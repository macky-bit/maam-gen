...
const LOGO_SVG = (
  <svg viewBox="0 0 111.81 30" className="h-6 w-auto fill-[var(--color-red)]" aria-label="StreamFlix">
...
function IconBtn({ label, children, active }: { label: string; children: React.ReactNode; active?: boolean }) {
  ...
  return (
    <button
      ...
      style={{ color: hov || active ? "var(--color-wine)" : "var(--color-cream)" }}
    >
      {children}
    </button>
  );
...
function Skip10Icon({ forward }: { forward?: boolean }) {
  return (
    <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ transform: forward ? "none" : "scaleX(-1)" }}>
      <polyline points="15 14 20 9 15 4" />
      <path d="M4 20v-7a4 4 0 0 1 4-4h12" />
      <text x="6" y="22" fontSize="6" fill="currentColor" stroke="none" fontFamily="sans-serif" fontWeight="bold">10</text>
    </svg>
  );
}
...
function Scrubber({ progress, onChange }: { progress: number; onChange: (v: number) => void }) {
  return (
        ...
        <div className="absolute bottom-0 left-0 right-0 px-5 pb-4 flex flex-col gap-2">
          ...
          <div className="flex items-center gap-4">
            <IconBtn label={playing ? "Pause" : "Play"} active={false}>
              <span onClick={() => setPlaying(p => !p)}><PlayPauseIcon playing={playing} /></span>
            </IconBtn>
            ...
            <IconBtn label="Volume"><VolumeIcon /></IconBtn>
            <IconBtn label="Subtitles"><CcIcon /></IconBtn>
            <IconBtn label="Settings"><SettingsIcon /></IconBtn>
            <IconBtn label="Fullscreen"><FullscreenIcon /></IconBtn>
          </div>
        </div>
      </div>
    ...
    <div className={`flex flex-col h-full rounded-xl overflow-hidden ${styles.episodePanel}`}>
      ...
      <div className={`px-4 pt-4 pb-3 ${styles.episodePanelHeader}`}>
        ...
        <div className={`flex items-center gap-3 mt-3 ${styles.episodePanelUtility}`}>
          <button className="hover:text-[var(--color-cream)] transition-colors" aria-label="Loop"><LoopIcon /></button>
          <button className="hover:text-[var(--color-cream)] transition-colors" aria-label="Shuffle"><ShuffleIcon /></button>
          <button className="ml-auto text-sm hover:text-[var(--color-cream)] transition-colors" aria-label="More">···</button>
        </div>
      </div>
    ...
    <div className={`flex gap-3 ${nested ? `pl-10 ${styles.commentThread}` : ""}`}>
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <button className={`text-xs transition-colors hover:text-[var(--color-wine)] ml-1 ${styles.replyBtn}`}>
            Reply
          </button>
        </div>
      </div>
...
function ReactionsAndComments({ contentType }: { contentType: "movie" | "series" }) {
