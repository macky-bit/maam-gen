...
export function Field({
	...
	return (
		<div className="flex flex-col gap-[7px]">
			<label
				className={`text-[10px] font-semibold uppercase tracking-[0.15em] ${styles.fieldLabel}`}
			>
				{label}
			</label>
            ...
            ${
				...
				{suffix && (
					<div
						className={`absolute right-3 top-1/2 -translate-y-1/2 ${styles.fieldSuffix}`}
					>
						{suffix}
					</div>
				)}
			...
			{error && (
				<p className={`text-[11px] ${styles.fieldError}`}>{error}</p>
			)}
		</div>
	);
}
		...
		<div className="flex gap-10">
			{[
				...
				<div key={label} className="flex flex-col gap-1">
					<span className={`text-2xl font-bold ${styles.statNum}`}>
						{num}
					</span>
					<span
						className={`text-[10px] uppercase tracking-[0.18em] ${styles.statLabel}`}
					>
						{label}
					</span>
				</div>
