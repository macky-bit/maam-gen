...
export function PlayIcon({ size = 16, color = "var(--color-cream)" }: { size?: number; color?: string }) {
  return (
  ...
  return (
    <section className="px-4 sm:px-10 xl:px-12">
      <div className="flex items-center gap-3 mb-3">
        ...
        {exploreAll && (
          <button className={`text-xs font-medium transition-colors hover:text-[var(--color-cream)] ${styles.exploreAllBtn}`}>
            Explore All &rsaquo;
          </button>
        )}
      </div>
    ...
    <header className={`fixed top-0 left-0 right-0 z-50 flex items-center gap-4 sm:gap-6 px-4 sm:px-10 xl:px-12 h-14 ${styles.header}`}>
      ...
      <div className="flex items-center gap-3 sm:gap-4">
        {searchOpen ? (
          ...
          <button onClick={() => setSearchOpen(true)} className={`transition-colors hover:text-[var(--color-cream)] ${styles.iconBtn}`} aria-label="Search">
            <SearchIcon />
          </button>
        )}
        ...
        <div className="relative">
          ...
          {profileOpen && (
            <div className={`absolute right-0 top-9 w-44 rounded-xl overflow-hidden shadow-2xl ${styles.dropdown}`}>
              {["Profile", "Account", "Settings", "Help Center"].map((item) => (
                <button
                  ...
                  onClick={() => {
                    ...
                    if (item === "Account" || item === "Settings") onNavigatePage?.("account");
              ...
              <button onClick={onSignOut} className={`w-full text-left px-4 py-2.5 text-sm transition-colors hover:bg-[var(--color-wine)]/30 ${styles.dropdownItemMuted}`}>
                Sign Out
              </button>
            </div>
          )}
    ...
    <footer className={`px-4 sm:px-10 xl:px-12 py-10 mt-8 ${styles.footer}`}>
      <div className="flex flex-wrap gap-x-5 gap-y-2 mb-4">
        {FOOTER_LINKS.map((link) => (
          <button key={link} className={`text-xs transition-colors hover:text-[var(--color-taupe)] ${styles.footerLink}`}>
            {link}
          </button>
        ))}
      </div>
