	...
	return (
		...
		>
			...
			{showProgress && show.progress !== undefined && (
				<div
					className={`absolute bottom-0 left-0 right-0 h-1 ${styles.progressTrack}`}
				>
					<div
			...
			{hovered && (
				<div
					className={`absolute inset-0 flex flex-col justify-end p-3 ${styles.cardOverlay}`}
				>
					<p
						className={`text-xs font-bold leading-tight mb-1 line-clamp-2 ${styles.cardTitle}`}
					>
						{show.title}
					</p>
					<div className="flex items-center gap-1.5 mb-2 flex-wrap">
						{typeof show.match === "number" && (
							<span
								className={`text-[10px] font-bold ${styles.cardMatch}`}
							>
								{show.match}%
							</span>
						)}
						<span className={`text-[10px] ${styles.cardMeta}`}>
							{show.year}
						</span>
						<span
							className={`text-[10px] px-1 border rounded ${styles.cardBadge}`}
						>
							{show.rating}
						</span>
					</div>
...
export function Top10Card({
	...
	return (
		...
		>
			...
			<div
				className="relative z-20 ml-10 rounded-lg overflow-hidden w-[100px] sm:w-[120px]"
	...
	return (
		<section className="px-4 sm:px-10 xl:px-12">
			<div className="flex items-center gap-3 mb-3">
				<h2 className={`text-lg font-bold ${styles.rowTitle}`}>{title}</h2>
				{exploreAll && (
					<button
						className={`text-xs font-medium transition-colors hover:text-[var(--color-cream)] ${styles.exploreAllBtn}`}
					>
						Explore All &rsaquo;
					</button>
				)}
...
export function Navbar({
	...
	return (
		...
		>
			...
			{mobileNavOpen && (
				<nav
					className={`md:hidden absolute top-14 left-0 right-0 flex flex-col py-2 ${styles.mobileNav}`}
				>
					{NAV_LINKS.map((link) => (
						<button
							key={link}
			...
			<div className="flex items-center gap-3 sm:gap-4">
				{searchOpen ? (
					...
					<button
						...
						className={`transition-colors hover:text-[var(--color-cream)] ${styles.iconBtn}`}
						aria-label="Search"
					>
						<SearchIcon />
					</button>
				...
				<div className="relative">
					<button
						...
						className={`relative transition-colors hover:text-[var(--color-cream)] ${styles.iconBtn}`}
				...
				<div className="relative">
					...
					>
						<span
							className={`w-7 h-7 rounded flex items-center justify-center text-xs font-bold ${styles.avatarBadge}`}
						>
							A
						</span>
					...
					{profileOpen && (
						<div
							className={`absolute right-0 top-9 w-44 rounded-xl overflow-hidden shadow-2xl ${styles.dropdown}`}
						>
	...
	return (
		...
		>
			...
			<div className="absolute bottom-16 sm:bottom-20 left-4 sm:left-10 xl:left-12 right-4 sm:right-auto max-w-[520px]">
				<p
					className={`text-[10px] uppercase tracking-[0.25em] mb-3 ${styles.heroEyebrow}`}
				>
				...
				<div className="flex gap-3">
					...
					<button
						className={`flex items-center gap-2 px-5 sm:px-6 py-3 rounded-lg text-sm font-bold transition-all duration-150 hover:bg-[var(--color-wine-hover)] active:scale-[0.98] ${styles.heroInfoBtn}`}
						onClick={() => onInfo(show)}
					>
			...
			<div className="absolute bottom-20 right-4 sm:right-10 xl:right-12 flex flex-col items-end gap-3">
				<button
					...
					className={`w-9 h-9 rounded-full flex items-center justify-center transition-colors hover:border-[var(--color-taupe)] ${styles.heroMuteBtn}`}
		...
		<footer className={`px-4 sm:px-10 xl:px-12 py-10 mt-8 ${styles.footer}`}>
			<div className="flex flex-wrap gap-x-5 gap-y-2 mb-4">
				{FOOTER_LINKS.map((link) => (
					<button
						key={link}
						className={`text-xs transition-colors hover:text-[var(--color-taupe)] ${styles.footerLink}`}
					>
						{link}
					</button>
				))}
