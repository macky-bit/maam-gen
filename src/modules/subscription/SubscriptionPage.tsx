	...
	return (
		...
		>
			<div
				...
				style={{ background: "var(--color-stone)", opacity: 0.4 }}
			/>
			<div
			...
			<div
				...
				style={{ background: "var(--color-stone)", opacity: 0.3 }}
			/>
			<div
		...
		>
			...
			>
				...
				<button
					...
					onMouseLeave={(e) =>
						((e.currentTarget as HTMLElement).style.color = "var(--color-taupe)")
					}
				>
				...
				{done ? (
					<div className="text-center py-4">
						...
						<p className="mb-6 text-sm" style={{ color: "var(--color-taupe)" }}>
							You selected the{" "}
							<span style={{ color: "var(--color-cream)" }}>{plan.PlanName}</span>{" "}
							plan.
						</p>
						<button
							onClick={onContinue}
					...
					<>
						...
						<div className="flex flex-col-reverse sm:flex-row gap-3">
							<button
								...
								onMouseEnter={(e) => {
									if (!loading)
										...
										).style.borderColor = "var(--color-taupe)";
								}}
								onMouseLeave={(e) => {
									if (!loading)
										(
							...
							<button
								...
								style={{
									background: loading ? "var(--color-wine)" : "var(--color-wine)",
									color: "var(--color-cream)",
									fontFamily: "'Inter', sans-serif",
								}}
								onMouseEnter={(e) => {
									if (!loading)
										(
											e.currentTarget as HTMLElement
										).style.background = "var(--color-wine-hover)";
								}}
								onMouseLeave={(e) => {
									if (!loading)
										(
											e.currentTarget as HTMLElement
										).style.background = loading ? "var(--color-wine)" : "var(--color-wine)";
								}}
							>
								{loading && (
									<span
										className="h-4 w-4 rounded-full border-2 border-transparent animate-spin"
										style={{ borderTopColor: "var(--color-cream)" }}
									/>
								)}
								{loading
									? "Processing…"
...
function PlanCard({
	...
	const cardBorderColor =
		selected || hovered ? "var(--color-wine)" : isStandard ? "var(--color-wine)" : "var(--color-stone)";
	const btnBg = isStandard ? "var(--color-wine)" : "transparent";
	const btnBorder = isStandard ? "var(--color-wine)" : "var(--color-stone)";
	...
	return (
		<div
			className={`flex flex-col rounded-sm transition-all duration-200 relative h-full ${flashing ? styles.cardFlash : ""}`}
			style={{
				...
				boxShadow: flashing
					? undefined
					: selected
						? `0 0 0 1px ${"var(--color-wine)"}, 0 8px 32px rgba(73,17,28,0.25)`
						: isStandard
		...
		>
			...
			<div className="flex flex-col items-center text-center px-7 pt-10 pb-7 flex-1 h-full">
				...
				<PlanIcon
					...
					style={{ color: "var(--color-taupe)" } as React.CSSProperties}
				/>
				...
				<div className="flex items-baseline gap-1.5 mb-4">
					...
					<span
						className="text-sm"
						style={{ color: "var(--color-taupe)", fontFamily: "'Inter', sans-serif" }}
					>
						/ month
					</span>
				</div>
				...
				<div
					...
					style={{ height: 1, background: "var(--color-stone)", opacity: 0.6 }}
				/>
				...
				<button
					...
					onMouseEnter={(e) => {
						(e.currentTarget as HTMLElement).style.background = "var(--color-wine-hover)";
						(e.currentTarget as HTMLElement).style.borderColor = "var(--color-wine)";
					}}
					onMouseLeave={(e) => {
						...
						(e.currentTarget as HTMLElement).style.borderColor =
							isStandard ? "var(--color-wine)" : "var(--color-stone)";
						setPressing(false);
					}}
					onFocus={(e) => {
...
export default function SubscriptionPage({ onComplete, onBack, onSubscribe }: Props) {
	...
	return (
		<div className={`min-h-full flex flex-col ${styles.page}`}>
			{/* ── MAIN ── */}
			<main className="flex-1 mx-auto w-full max-w-5xl px-6 py-14 md:px-10">
				...
				<header className="mb-12 text-center">
					...
					<p className="text-base" style={{ color: "var(--color-taupe)" }}>
				...
				{pageState === "ready" && (
					<>
						...
						<div className="mt-4 text-center">
							<button type="button" className={`text-sm underline underline-offset-2 transition-colors ${styles.backLink}`} onClick={onBack}>
								Back to StreamFlix
							</button>
						</div>
					</>
