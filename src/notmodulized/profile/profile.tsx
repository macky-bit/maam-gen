...
function Navbar({ onManageProfiles }: { onManageProfiles: () => void }) {
	...
	return (
		<nav
			className="fixed top-0 left-0 right-0 z-50 bg-[#0A0908]"
			style={{ borderBottom: "1px solid rgba(94,80,63,0.3)" }}
		>
							...
							{items.map((item) => (
								<button
									...
									onMouseEnter={(e) => {
										...
										).style.background = "rgba(73,17,28,0.25)";
									}}
									onMouseLeave={(e) => {
										(
											e.currentTarget as HTMLElement
								...
								>
									{item === "Profile" && (
										<span className="inline-block w-1.5 h-1.5 rounded-full bg-[#49111C] mr-2 align-middle" />
									)}
									{item}
								</button>
							))}
		...
		>
			...
			>
				<div className="flex items-center justify-between mb-6">
					<h2
						className="text-[#F2F4F3] font-display font-bold text-xl tracking-wide uppercase"
						style={{ fontFamily: "'Barlow Condensed', sans-serif" }}
					>
						Edit Profile
					</h2>
					<button
						onClick={onClose}
						className="text-[#A9927D] hover:text-[#F2F4F3] transition-colors p-1"
					>
						<XIcon />
					</button>
				</div>
				...
				<div className="flex items-center gap-4 mb-6">
					...
					<button
						className="text-sm text-[#A9927D] hover:text-[#F2F4F3] border border-[#5E503F] hover:border-[#A9927D] px-3 py-1.5 rounded-sm transition-colors"
				...
				<div className="space-y-4">
					<div>
						<label
							className="block text-xs text-[#A9927D] mb-1.5 uppercase tracking-wider"
							style={{ fontFamily: "'Barlow Condensed', sans-serif" }}
						>
							Profile Name
						</label>
	...
	return (
		...
		>
			...
			>
				...
				<p
					className="text-[#A9927D] text-sm mb-5"
					style={{ fontFamily: "'Barlow', sans-serif" }}
				>
				...
				{saved && (
					<p
						className="text-[#49111C] text-sm mt-3"
						style={{ fontFamily: "'Barlow', sans-serif" }}
					>
		...
		>
			...
			>
				...
				<div className="space-y-1 max-h-80 overflow-y-auto">
					{items.length === 0 && (
						<p
							className="text-[#A9927D] text-sm py-4 text-center"
							style={{ fontFamily: "'Barlow', sans-serif" }}
						>
							No watch history.
						</p>
					...
					{items.map((item) => (
						...
						>
							...
							<div className="flex-1 min-w-0">
								<p
									className="text-[#F2F4F3] text-sm font-medium truncate"
									style={{ fontFamily: "'Barlow', sans-serif" }}
								>
									{item.title}
								</p>
								<p
									className="text-[#A9927D] text-xs"
									style={{ fontFamily: "'Barlow', sans-serif" }}
								>
									{item.subtitle}
								</p>
								<p
									className="text-[#5E503F] text-xs mt-0.5"
									style={{ fontFamily: "'Barlow', sans-serif" }}
								>
									{item.date}
								</p>
			...
			>
				...
				<div className="space-y-1 mb-4">
					{profiles.map((p) => (
						<div
							key={p.id}
							className="flex items-center gap-3 py-2.5 rounded-sm px-2 hover:bg-[rgba(73,17,28,0.12)] transition-colors cursor-pointer"
							style={{ borderBottom: "1px solid rgba(94,80,63,0.15)" }}
						>
							<div
								className="w-9 h-9 bg-[#49111C] text-[#F2F4F3] font-bold flex items-center justify-center rounded-sm text-sm flex-shrink-0"
								style={{ fontFamily: "'Barlow Condensed', sans-serif" }}
							>
								{p.initials}
							</div>
							...
							<button
								className="text-[#5E503F] hover:text-[#A9927D] transition-colors p-1"
				...
				{adding ? (
					<div className="flex gap-2 mb-4">
						...
						<button
							onClick={addProfile}
							className="px-4 py-2 bg-[#49111C] text-[#F2F4F3] text-sm rounded-sm hover:bg-[#3a0d16] transition-colors"
							style={{ fontFamily: "'Barlow', sans-serif" }}
						>
							Add
						</button>
						<button
							...
							className="px-4 py-2 border border-[#5E503F] text-[#F2F4F3] text-sm rounded-sm hover:border-[#A9927D] transition-colors"
							style={{ fontFamily: "'Barlow', sans-serif" }}
						>
							Cancel
						</button>
				...
				<button
					onClick={onClose}
					className="w-full py-2.5 border border-[#5E503F] text-[#F2F4F3] text-sm rounded-sm hover:border-[#A9927D] transition-colors"
					style={{ fontFamily: "'Barlow', sans-serif" }}
				>
					Done
				</button>
...
function ContinueWatchingCard({
	...
	return (
		<div
			className="card-hover relative rounded-sm overflow-hidden flex-shrink-0 transition-transform duration-200 cursor-pointer"
		...
		>
			...
			<div
				...
				style={{
					background:
						"linear-gradient(to top, rgba(10,9,8,0.95) 0%, rgba(10,9,8,0.3) 50%, transparent 100%)",
				}}
			/>

			{/* Hover overlay */}
			<div
				className="card-overlay absolute inset-0 flex items-center justify-center gap-2 opacity-0 transition-opacity duration-200"
				style={{ background: "rgba(10,9,8,0.5)" }}
			>
				<button
					className="bg-[#F2F4F3] text-[#0A0908] rounded-full p-2 hover:bg-[#A9927D] transition-colors"
					aria-label={`Play ${card.title}`}
				>
					<PlayIcon size={16} />
				</button>
				<button
					className="border border-[rgba(242,244,243,0.6)] text-[#F2F4F3] rounded-full p-2 hover:border-[#F2F4F3] transition-colors"
			...
			<div className="absolute bottom-0 left-0 right-0 px-3 pb-2 pt-6">
				<p
					className="text-[#F2F4F3] text-sm font-medium mb-2 truncate"
					style={{ fontFamily: "'Barlow', sans-serif" }}
				>
					{card.title}
				</p>
				<div
					...
					style={{ background: "#5E503F" }}
				>
					<div
						className="h-full rounded-full bg-[#49111C]"
...
function SettingRow({
	...
	return (
		<div
			className={`flex items-center gap-4 py-4 ${!last ? "border-b border-[rgba(94,80,63,0.2)]" : ""}`}
		>
			<div className="text-[#A9927D] flex-shrink-0">{icon}</div>
			<div className="flex-1 min-w-0">
				<p
					className="text-[#F2F4F3] text-sm font-medium"
					style={{ fontFamily: "'Barlow', sans-serif" }}
				>
					{label}
				</p>
				<p
					className="text-[#A9927D] text-xs mt-0.5 leading-relaxed"
					style={{ fontFamily: "'Barlow', sans-serif" }}
				>
					{description}
				</p>
			...
			<div className="max-w-7xl mx-auto px-6 md:px-10 pt-[72px]">
				<div className="py-10">
					...
					<div className="flex flex-wrap items-center gap-5 mb-10">
						...
						<div className="flex-1 min-w-0">
							<h2
								className="text-[#F2F4F3] font-display font-bold text-2xl tracking-wide"
								style={{ fontFamily: "'Barlow Condensed', sans-serif" }}
							>
								Kevin Chan
							</h2>
							<p
								className="text-[#A9927D] text-sm mt-0.5"
								style={{ fontFamily: "'Barlow', sans-serif" }}
							>
								Primary Profile
							</p>
