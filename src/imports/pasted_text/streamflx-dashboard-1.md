...
## 14. REACTIONS & COMMENTS (below the player)

Add this directly beneath the title/metadata block on the **Watch Screen** (§13), for both Movie and Series variants. Static mockup — no live comment posting, just the visual states.

### Reactions row
Centered block, stacked:
- Small heading: "What did you think of this [movie/episode]?" — `#F2F4F3`, semibold
- Below it, small total count e.g. "1.9K reactions" — `#A9927D`, small
- Row of 6 emoji reaction buttons, evenly spaced, each showing: emoji icon, count number (`#F2F4F3`, bold), label beneath (`#A9927D`, small) — Upvote 👍 · Funny 😂 · Love ❤️ · Surprised 😮 · Angry 😠 · Sad 😢
- Reaction icons sit on transparent background; no border. Keep generous spacing between each (~40–56px)

### Comments header
- Left: "[N] Comments" — `#F2F4F3`, semibold
- Right: sort tabs — **Best** (active pill: bg `#49111C`, text `#F2F4F3`) · Newest · Oldest (inactive: `#A9927D` text, no background, `#49111C` on hover)

### Comment composer (logged-out state)
- Full-width input box, bg `#0A0908` slightly lifted (e.g. `#5E503F` at low opacity as fill, or a subtle 1px `#5E503F` border), rounded corners ~8px
- Placeholder text: "Sign up to join the discussion…" — `#A9927D`
- Bottom toolbar row inside the box: basic formatting icons (Bold, Italic, spoiler/blur toggle) on the left in muted `#5E503F` / `#A9927D` (disabled-looking, since posting requires sign-up), and a **Sign Up to Comment** primary button on the right — bg `#49111C`, text `#F2F4F3`, small person/plus icon

### Rules notice
Small dismissible bar below the composer:
- Text: "By commenting, you agree to follow our comment rules." — `#A9927D`
- **Read Rules** button — bg `#49111C`, text `#F2F4F3`, small, rounded
- Small "×" dismiss icon on the far right — `#A9927D`
- Background slightly darker/bordered panel to visually separate it from the composer above (thin `#5E503F` border)

### Comment thread (logged-in / populated state — build as a second frame variant)
For the Components page, also design one populated comment row:
- Avatar circle (left)
- Username (`#F2F4F3`, semibold) + timestamp (`#A9927D`, small) on the same line
- Comment text below (`#F2F4F3`)
- Action row beneath: reaction icons with counts (reuse the same 6 reaction icons, smaller scale) + "Reply" text link (`#A9927D`, `#49111C` on hover)
- Nested replies indent ~32–40px with a thin `#5E503F` left connector line

---

## 15. FIGMA FILE ORGANIZATION — IMPORTANT
...
**04 — WATCH SCREEN:** The static player mockup from §13 — Movie variant and Series variant (with episode panel) side by side for comparison, plus the reactions & comments block from §14 shown beneath each.

**05 — COMPONENTS:** Dedicated design-system page. Build these as **true Figma components with variants** (Default / Hover / Active / Selected / Disabled) — not duplicated static frames. Include: StreamFlix Logo, Buttons, Input fields, Password fields, Checkboxes, Navigation items, Movie cards, Genre pills, Progress bars, Icons, Dropdowns, Modals, Cards, Dividers, Episode list row, Player controls, Reaction button (with count), Comment composer, Comment row (default + with replies), Sort tab.
